...
class BertForMultiLabelClassification(nn.Module):
    def __init__(self, num_labels, dim_hidden):
        super(BertForMultiLabelClassification, self).__init__()
        self.bert = BertModel.from_pretrained('bert-base-uncased')
        self.dropout = nn.Dropout(0.1)
        self.dense = nn.Linear(self.bert.config.hidden_size, dim_hidden)
        self.relu = nn.ReLU()
        self.classifier = nn.Linear(dim_hidden, num_labels)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        hidden_layer = self.dense(pooled_output)
        hidden_layer = self.relu(hidden_layer)
        return self.classifier(hidden_layer)
